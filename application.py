...
@application.route("/api/products/search", methods=["POST"])
def search_products():
    data = request.json
    if "query" in data:
        products = Product.query.filter(Product.name.contains(data["query"])).all()
        product_list = []
        for product in products:
            product_data = {
                "id": product.id,
...
def checkout():
    ...
    return jsonify({"message": "Failed to checkout"}), 400
